...
type Tab = "registrations" | "competitions";
...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const [regRes, statsRes, compRes] = await Promise.all([
      getRegistrations({
    ...
    const students = await getStudents(regRes.data.map((reg) => reg.uid));
    if (regRes.success && students.success) {
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8 max-w-7xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <Link href="/dashboard/admin/competitions/create">
              <Button variant="primary" className="gap-2">
                <svg
                  className="w-4 h-4"
                  fill="none"
        ...
        <div className="flex items-center gap-1 mb-6 border-b border-border">
          {(["registrations", "competitions"] as Tab[]).map((tab) => (
            <button
              key={tab}
                            ...
                            <TableRow key={c.id}>
                              ...
                              <TableCell className="text-sm">
                                {c.startDate && c.endDate
                                  ? `${Math.round(
                                      (new Date(c.endDate as string).getTime() -
                                        new Date(
                                          c.startDate as string,
                                        ).getTime()) /
                                        (60 * 60 * 1000),
                              ...
                              <TableCell className="text-right">
                                <div className="flex items-center justify-end gap-2">
                                  <Link
                                    href={`/dashboard/admin/competitions/${c.id}/edit`}
                                  >
                                  ...
                                  <Button
                                    ...
                                    variant="danger"
                                    onClick={() =>
                                      handleDeleteCompetition(c.id as string)
                                    }
                                  >
                                    Delete
                                  </Button>
